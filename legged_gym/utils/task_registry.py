...
class TaskRegistry:
    ...
    def make_env(self, name, args=None, env_cfg=None):
        ...
        sim_device = "cpu" if args.cpu else "mps:0"  # "cuda:0"
        # sim_params
        sim_params = class_to_dict(env_cfg.sim)
        env = task_class(cfg=env_cfg, sim_params=sim_params, sim_device=sim_device, headless=args.headless)
        return env, env_cfg
    ...
    def make_alg_runner(self, env, name=None, args=None, train_cfg=None, log_root="default"):
        ...
        sim_device = "cpu" if args.cpu else "mps"  # "cuda"
        ...
        if resume:
            if train_cfg.runner_class_name == "OnPolicyRunnerEE":
                model_resume_path, estimator_resume_path = get_load_path_ee(
