...
def parse_device_str(device_str):
    ...
    if device_str == "cpu" or device_str == "gpu" or device_str == "cuda" or device_str == "mps":
        device = device_str
        device_id = 0
    else:
...
def parse_arguments(description="Isaac Gym Example", headless=False, no_graphics=False, custom_parameters=[]):
    ...
    if no_graphics:
        parser.add_argument(
            "--nographics", action="store_true", help="Disable graphics context creation, no viewer window is created, and no headless rendering is available"
        )
...
def parse_sim_config(sim_cfg: dict, sim_options):
...
def parse_flex_config(flex_cfg: dict, sim_options):
...
def parse_physx_config(physx_cfg: dict, sim_options):
...
def generate_random_samples(attr_randomization_params, shape, randomization_ct, extern_sample=None):
...
def apply_random_samples(prop, og_prop, attr, attr_randomization_params, randomization_ct, extern_sample=None):
    if isinstance(prop, gymapi.SimParams):
        ...
        sample = generate_random_samples(attr_randomization_params, prop[attr].shape, randomization_ct, extern_sample)
        if attr_randomization_params["operation"] == "scaling":
        ...
        sample = generate_random_samples(attr_randomization_params, 1, randomization_ct, extern_sample)
...
def check_buckets(gym, envs, dr_params):
    ...
    ), "Explicit material bucketing has been specified, but the provided total bucket count exceeds 64K: {} specified buckets".format(total_num_buckets)
