...
# from legged_gym.utils.torch_utils import gymutil, gymapi
...
def random_uniform_terrain(
    ...
    height_field_downsampled = np.random.choice(
        heights_range, (int(terrain.width * terrain.horizontal_scale / downsampled_scale), int(terrain.length * terrain.horizontal_scale / downsampled_scale))
    )
...
def warp_uniform_terrain(
    ...
    # lenth = int(lenth / terrain.horizontal_scale)
    edge_width = int(width / 20)
    edge_lenth = int(lenth / 20)

    terrain.height_field_raw[edge_width * 2 : width - edge_width, edge_lenth : lenth - edge_lenth] = height
...
def wave_terrain(terrain, num_waves=1, amplitude=1.0):
    ...
    if num_waves > 0:
        ...
        xx = xx.reshape(terrain.width, 1)
        yy = yy.reshape(1, terrain.length)
        terrain.height_field_raw += (amplitude * np.cos(yy / div) + amplitude * np.sin(xx / div)).astype(terrain.height_field_raw.dtype)
    return terrain
