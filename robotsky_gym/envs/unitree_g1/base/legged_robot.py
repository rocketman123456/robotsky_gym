...
from robotsky_gym.envs.g1.base.base_task import BaseTask
from robotsky_gym.utils.math import wrap_to_pi, torch_rand_sqrt_float, quat_apply_yaw
...
class LeggedRobot(BaseTask):
    ...
    def _prepare_reward_function(self):
        ...
        # reward episode sums
        self.episode_sums = {name: torch.zeros(self.num_envs, dtype=gs.tc_float, device=self.device, requires_grad=False) for name in self.reward_scales.keys()}
    ...
    def _reward_collision(self):
        ...
        return torch.sum(1.0 * (torch.norm(self.link_contact_forces[:, self.penalized_indices, :], dim=-1) > 0.1), dim=1)
