...
class OnPolicyRunnerDagger(OnPolicyRunner):
    ...
    def __init__(self, env: VecEnv, train_cfg, log_dir=None, device="cpu"):
        ...
        self.enable_dagger = self.alg_cfg["enable_dagger"]
        self.evaluate_expert_teacher = self.alg_cfg["evaluate_expert_teacher"]
        ...
        if self.enable_dagger:
            actor_critic_expert: ActorCritic = actor_critic_class(
                self.env.num_actor_obs,
                self.env.num_critic_obs,
                self.env.enable_env_encoder,
                self.env.num_privileged_obs,
        ...
        if self.enable_dagger:
            if self.multi_critics:
                alg_class = eval(self.cfg["algorithm_class_name"] + "Dagger" + "MultiCritic")  # PPO
            else:
                alg_class = eval(self.cfg["algorithm_class_name"] + "Dagger")  # PPO
        ...
        # disable grad for expert
        if self.enable_dagger:
            if self.multi_critics:
                for param in self.alg.actor_critic_expert.critic_1.parameters():
                    param.requires_grad = False
                for param in self.alg.actor_critic_expert.critic_2.parameters():
                    param.requires_grad = False
            else:
                for param in self.alg.actor_critic_expert.critic.parameters():
                    param.requires_grad = False
            for param in self.alg.actor_critic_expert.actor.parameters():
                param.requires_grad = False
            for param in self.alg.actor_critic_expert.adaptation_module.parameters():
                param.requires_grad = False
            for param in self.alg.actor_critic_expert.env_encoder.parameters():
                param.requires_grad = False
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    ...
                    #     if self.env.enable_env_encoder:
                    #         if self.env.evaluate_teacher:
                    #             actions_expert = self.alg.act_with_env_encoder_expert(actor_obs, critic_obs, privileged_obs, obs_history, action_history)
                    #         else:
                    #             actions_expert = self.alg.act_with_adaptation_module_expert(actor_obs, critic_obs, privileged_obs, obs_history, action_history)
                    #     else:
                    #         actions_expert = self.alg.act_expert(actor_obs, critic_obs)
                ...
                if self.env.enable_env_encoder:
                    if self.env.evaluate_teacher:
                        ...
                        # self.alg.compute_returns_with_env(critic_obs, privileged_obs)
                        self.alg.compute_returns_with_adaptation(critic_obs, obs_history)
                else:
                ...
                if self.enable_dagger:
                    ...
                    if self.env.enable_env_encoder:
                        if self.env.evaluate_teacher:
                            self.alg.compute_returns_with_env_expert(critic_obs, privileged_obs)
                        else:
                            # self.alg.compute_returns_with_env_expert(critic_obs, privileged_obs)
                            self.alg.compute_returns_with_adaptation_expert(critic_obs, obs_history)
                    else:
                        self.alg.compute_returns_expert(critic_obs)
    ...
    def load_expert(self, path, load_optimizer=True):
        loaded_dict = torch.load(path)
        self.alg.actor_critic_expert.load_state_dict(loaded_dict["model_state_dict"])
        ...
        if self.enable_dagger:
            ...
            self.alg.actor_critic.env_encoder = copy.deepcopy(self.alg.actor_critic_expert.env_encoder)
            self.alg.actor_critic.adaptation_module = copy.deepcopy(self.alg.actor_critic_expert.adaptation_module)
