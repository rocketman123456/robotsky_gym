...
class PPODaggerMultiCritic(PPO):
    ...
    def act_expert(self, actor_obs, critic_obs):
        ...
        if self.multi_critics:
            self.transition.values1 = self.actor_critic_expert.evaluate1(critic_obs).detach()
            self.transition.values2 = self.actor_critic_expert.evaluate2(critic_obs).detach()
    ...
    def act_with_env_encoder_expert(self, actor_obs, critic_obs, privileged_obs, obs_history, action_history):
        ...
        if self.multi_critics:
            self.transition.values1 = self.actor_critic_expert.evaluate1_with_env(critic_obs, privileged_obs).detach()
            self.transition.values2 = self.actor_critic_expert.evaluate2_with_env(critic_obs, privileged_obs).detach()
        self.transition.actions_log_prob = self.actor_critic_expert.get_actions_log_prob(self.transition.actions).detach()
        self.transition.action_mean = self.actor_critic_expert.action_mean.detach()
        self.transition.action_sigma = self.actor_critic_expert.action_std.detach()
        return self.transition.actions
    ...
    def act_with_adaptation_module_expert(self, actor_obs, critic_obs, privileged_obs, obs_history, action_history):
        ...
        if self.multi_critics:
            self.transition.values1 = self.actor_critic_expert.evaluate1_with_adaptation(critic_obs, obs_history).detach()
            self.transition.values2 = self.actor_critic_expert.evaluate2_with_adaptation(critic_obs, obs_history).detach()
        self.transition.actions_log_prob = self.actor_critic_expert.get_actions_log_prob(self.transition.actions).detach()
        self.transition.action_mean = self.actor_critic_expert.action_mean.detach()
        self.transition.action_sigma = self.actor_critic_expert.action_std.detach()
        return self.transition.actions
    ...
    def compute_returns_expert(self, last_critic_obs):
        last_values1 = self.actor_critic_expert.evaluate1(last_critic_obs).detach()
        last_values2 = self.actor_critic_expert.evaluate2(last_critic_obs).detach()
        self.storage.compute_multi_returns(last_values1, last_values2, self.gamma, self.lam)

    def compute_returns_with_env_expert(self, last_critic_obs, last_privileged_obs):
        last_values1 = self.actor_critic_expert.evaluate1_with_env(last_critic_obs, last_privileged_obs).detach()
        last_values2 = self.actor_critic_expert.evaluate2_with_env(last_critic_obs, last_privileged_obs).detach()
        self.storage.compute_multi_returns(last_values1, last_values2, self.gamma, self.lam)  # gamma 0.99 lam 0.95

    def compute_returns_with_adaptation_expert(self, last_critic_obs, last_obs_history):
        last_values1 = self.actor_critic_expert.evaluate1_with_adaptation(last_critic_obs, last_obs_history).detach()
        last_values2 = self.actor_critic_expert.evaluate2_with_adaptation(last_critic_obs, last_obs_history).detach()
        self.storage.compute_multi_returns(last_values1, last_values2, self.gamma, self.lam)
    ...
    def update(self):
        ...
        for (
            ...
            if self.enable_env_encoder:
                ...
                value1_batch = self.actor_critic.evaluate1(
                    critic_obs_batch,
                    masks=masks_batch,
                    hidden_states=hid_states_batch[1],
                )
                value2_batch = self.actor_critic.evaluate2(
                    critic_obs_batch,
                    masks=masks_batch,
                    hidden_states=hid_states_batch[1],
                )
