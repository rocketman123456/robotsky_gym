...
class PPOMultiCritic(PPO):
    ...
    def __init__(
        ...
        #     self.actor_critic.memory_a.reset_full(self.num_envs)
        #     self.actor_critic.memory_c.reset_full(self.num_envs)
    ...
    def act(self, actor_obs, critic_obs):
        ...
        if self.multi_critics:
            self.transition.values1 = self.actor_critic.evaluate1(critic_obs).detach()
            self.transition.values2 = self.actor_critic.evaluate2(critic_obs).detach()
    ...
    def act_with_env_encoder(self, actor_obs, critic_obs, privileged_obs, obs_history, action_history):
        ...
        if self.multi_critics:
            self.transition.values1 = self.actor_critic.evaluate1_with_env(critic_obs, privileged_obs).detach()
            self.transition.values2 = self.actor_critic.evaluate2_with_env(critic_obs, privileged_obs).detach()
        self.transition.actions_log_prob = self.actor_critic.get_actions_log_prob(self.transition.actions).detach()
        self.transition.action_mean = self.actor_critic.action_mean.detach()
        self.transition.action_sigma = self.actor_critic.action_std.detach()
        return self.transition.actions
    ...
    def act_with_adaptation_module(self, actor_obs, critic_obs, privileged_obs, obs_history, action_history):
        ...
        if self.multi_critics:
            self.transition.values1 = self.actor_critic.evaluate1_with_adaptation(critic_obs, obs_history).detach()
            self.transition.values2 = self.actor_critic.evaluate2_with_adaptation(critic_obs, obs_history).detach()
        self.transition.actions_log_prob = self.actor_critic.get_actions_log_prob(self.transition.actions).detach()
        self.transition.action_mean = self.actor_critic.action_mean.detach()
        self.transition.action_sigma = self.actor_critic.action_std.detach()
        return self.transition.actions

    def process_env_step(self, rewards1, rewards2, dones, infos):
        self.transition.rewards1 = rewards1.clone()
        self.transition.rewards2 = rewards2.clone()
        self.transition.dones = dones
        # Bootstrapping on time outs
        if "time_outs" in infos:
            self.transition.rewards1 += self.gamma * torch.squeeze(self.transition.values1 * infos["time_outs"].unsqueeze(1).to(self.device), 1)
            self.transition.rewards2 += self.gamma * torch.squeeze(self.transition.values2 * infos["time_outs"].unsqueeze(1).to(self.device), 1)
    ...
    def compute_returns(self, last_critic_obs):
        last_values1 = self.actor_critic.evaluate1(last_critic_obs).detach()
        last_values2 = self.actor_critic.evaluate2(last_critic_obs).detach()
        self.storage.compute_multi_returns(last_values1, last_values2, self.gamma, self.lam)

    def compute_returns_with_env(self, last_critic_obs, last_privileged_obs):
        last_values1 = self.actor_critic.evaluate1_with_env(last_critic_obs, last_privileged_obs).detach()
        last_values2 = self.actor_critic.evaluate2_with_env(last_critic_obs, last_privileged_obs).detach()
        self.storage.compute_multi_returns(last_values1, last_values2, self.gamma, self.lam)  # gamma 0.99 lam 0.95

    def compute_returns_with_adaptation(self, last_critic_obs, last_obs_history):
        last_values1 = self.actor_critic.evaluate1_with_adaptation(last_critic_obs, last_obs_history).detach()
        last_values2 = self.actor_critic.evaluate2_with_adaptation(last_critic_obs, last_obs_history).detach()
        self.storage.compute_multi_returns(last_values1, last_values2, self.gamma, self.lam)
    ...
    def update(self):
        ...
        if self.actor_critic.is_recurrent:
            generator = self.storage.reccurent_mini_batch_generator_multi_critics(self.num_mini_batches, self.num_learning_epochs)
        else:
            generator = self.storage.mini_batch_generator_multi_critics(self.num_mini_batches, self.num_learning_epochs)
        ...
        for (
            ...
            target_values1_batch,
            target_values2_batch,
            advantages1_batch,
            advantages2_batch,
            returns1_batch,
            returns2_batch,
            ...
            if self.enable_env_encoder:
                if self.evaluate_teacher:
                    value1_batch = self.actor_critic.evaluate1_with_env(
                        critic_obs_batch, privileged_obs_batch, masks=masks_batch, hidden_states=hid_states_batch[1]
                    )
                    value2_batch = self.actor_critic.evaluate2_with_env(
                        critic_obs_batch, privileged_obs_batch, masks=masks_batch, hidden_states=hid_states_batch[1]
                    )
                else:
                    value1_batch = self.actor_critic.evaluate1_with_adaptation(
                        critic_obs_batch, obs_history_batch, masks=masks_batch, hidden_states=hid_states_batch[1]
                    )
                    value2_batch = self.actor_critic.evaluate2_with_adaptation(
                        critic_obs_batch, obs_history_batch, masks=masks_batch, hidden_states=hid_states_batch[1]
                    )
            else:
                value1_batch = self.actor_critic.evaluate1(critic_obs_batch, masks=masks_batch, hidden_states=hid_states_batch[1])
                value2_batch = self.actor_critic.evaluate2(critic_obs_batch, masks=masks_batch, hidden_states=hid_states_batch[1])
            ...
            surrogate = -torch.squeeze(advantages1_batch + advantages2_batch) * ratio
            surrogate_clipped = -torch.squeeze(advantages1_batch + advantages2_batch) * torch.clamp(ratio, 1.0 - self.clip_param, 1.0 + self.clip_param)
            surrogate_loss = torch.max(surrogate, surrogate_clipped).mean()

            # Value function loss
            if self.use_clipped_value_loss:
                value1_clipped = target_values1_batch + (value1_batch - target_values1_batch).clamp(-self.clip_param, self.clip_param)
                value1_losses = (value1_batch - returns1_batch).pow(2)
                value1_losses_clipped = (value1_clipped - returns1_batch).pow(2)
                value1_loss = torch.max(value1_losses, value1_losses_clipped).mean()

                value2_clipped = target_values2_batch + (value2_batch - target_values2_batch).clamp(-self.clip_param, self.clip_param)
                value2_losses = (value2_batch - returns2_batch).pow(2)
                value2_losses_clipped = (value2_clipped - returns2_batch).pow(2)
                value2_loss = torch.max(value2_losses, value2_losses_clipped).mean()
            else:
                value1_loss = (returns1_batch - value1_batch).pow(2).mean()
                value2_loss = (returns2_batch - value2_batch).pow(2).mean()
            ...
            loss = (
                surrogate_loss
                + self.value_loss_coef * (value1_loss + value2_loss)
            ...
            mean_value_loss += (value1_loss + value2_loss).item()
