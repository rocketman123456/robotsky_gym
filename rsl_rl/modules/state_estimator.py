...
class VAE(nn.Module):
    def __init__(
        ...
        modules.extend([nn.Linear(decoder_input_dim, decoder_hidden_dims[0]), activation_fn])
...
class MLPHistoryEncoder(nn.Module):
    def __init__(
        ...
        for i in range(len(adaptation_module_branch_hidden_dims)):
            if i == len(adaptation_module_branch_hidden_dims) - 1:
                adaptation_module_layers.append(nn.Linear(adaptation_module_branch_hidden_dims[i], output_size))
            else:
                adaptation_module_layers.append(nn.Linear(adaptation_module_branch_hidden_dims[i], adaptation_module_branch_hidden_dims[i + 1]))
                adaptation_module_layers.append(activation)
